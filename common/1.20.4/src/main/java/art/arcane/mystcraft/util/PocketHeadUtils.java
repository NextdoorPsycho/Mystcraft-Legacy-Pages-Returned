...
/**
 * NeoForge 1.20.4+ version of PocketHeadUtils.
 * Uses the updated MinecraftSessionService.getTextures API (single argument).
 */
  ...
  private static String getSkinUrl(MinecraftServer server, GameProfile profile) {
    ...
    if (textures == null) {
      try {
        ...
        // 1.20.4+ API: getTextures returns MinecraftProfileTextures object
        com.mojang.authlib.minecraft.MinecraftProfileTextures profileTextures = session.getTextures(profile);
        if (profileTextures != null) {
          com.mojang.authlib.minecraft.MinecraftProfileTexture skin = profileTextures.skin();
