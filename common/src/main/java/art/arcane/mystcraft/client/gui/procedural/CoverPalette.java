 ...
 * <p>
 * This is a hand-curated table covering the items the Book Binder accepts
 * by default ({@code minecraft:leather}, {@code mystcraft:folder}) plus
 * the common alternatives players are likely to configure. Unknown items
 * fall back to the leather palette so generation never fails.
 */
public final class CoverPalette {
   ...
   * @param accentColor  Secondary color used for grain overlays / spine shading.
   * @param trimColor    Color of the spine trim, ribbon, and bookmark.
   * @param grain        Grain style to overlay on the base color.
   */
  public record Entry(int baseColor, int accentColor, int trimColor, Grain grain) {

    public Entry {
      // Defensive: clamp colors to RGB range (no alpha leakage).
      baseColor &= 0xFFFFFF;
  ...
  static {
    ...
    // Wood planks (common config picks)
  ...
  private static void register(@NotNull String id, @NotNull Entry entry) {
    ENTRIES.put(new ResourceLocation(id), entry);
  }
   ...
   */
  public static void register(@NotNull ResourceLocation id, @NotNull Entry entry) {
    ENTRIES.put(id, entry);
  }

  /**
   * Looks up a cover entry by item ID. Returns {@link #defaultEntry()} if
   * the item has no registered palette so that generation never fails for
   * configured cover items the palette doesn't know about.
   */
  @NotNull
  public static Entry get(@Nullable ResourceLocation coverItemId) {
  ...
  public static boolean hasEntry(@Nullable ResourceLocation coverItemId) {
