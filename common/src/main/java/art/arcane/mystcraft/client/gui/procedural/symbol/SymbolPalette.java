 ...
 * <p>
 * Defaults are baked in (matching the plan's §5.3.4 table). Resource packs
 * can override per-category palettes by shipping
 ...
 * <p>
 * Symbol-level overrides (per-symbol motif / palette pin) live on the
 * datapack {@code display} block (phase 4) and resolve through the same
 * lookup path via {@link #styleFor}.
 */
public final class SymbolPalette {
   ...
   * @param haloColor    Rank-5 halo + warning halo for high-instability symbols.
   * @param defaultMotif Layout template for symbols in this category.
   */
  public record Entry(int baseColor, int accentColor, int inkColor, int haloColor, SymbolMotif defaultMotif) {

    public Entry {
      // Strip any alpha leakage so callers always get clean RGB triples.
      baseColor &= 0xFFFFFF;
  ...
  /** Fallback used when an unknown / null category is queried. */
  ...
  private static Map<SymbolCategory, Entry> buildDefaults() {
    ...
    // Mirrors plan §5.3.4 table. Default motif column resolves through
    // SymbolMotif.defaultFor so the per-category motif assignment lives
    // in one place (plan §5.3.2).
  ...
  private static Entry paletteFor(@NotNull SymbolCategory c) {
    ...
    return switch (c) {
      ...
      case BIOME_CONTROLLER -> new Entry(0x2F5A3E, 0x1B3725, 0x081A12, 0x86C97A, motif);
      ...
      case VISUAL_EFFECT    -> new Entry(0xC95A8C, 0x712C4F, 0x2C0E1F, 0xFAB0D2, motif);
      case ENVIRONMENT      -> new Entry(0x603F8C, 0x331A56, 0x0E0826, 0xC9B0FA, motif);
      case FEATURE_LARGE    -> new Entry(0x8E5A2C, 0x4F2E12, 0x1F0E04, 0xE6B080, motif);
      case FEATURE_MEDIUM   -> new Entry(0xA07033, 0x583A14, 0x241405, 0xF0C58E, motif);
      case FEATURE_SMALL    -> new Entry(0xB39764, 0x6E5A2C, 0x2C2008, 0xF7E4B0, motif);
      case STRUCTURE        -> new Entry(0x35499D, 0x1A256B, 0x080F2A, 0xA8B7FA, motif);
  ...
  /**
   * Looks up the palette entry for the given category. Returns the
   * built-in fallback (neutral grey + DIAMOND motif) when the category
   * is null or has no registered entry.
   */
  @NotNull
  ...
  /**
   * Loads palette overrides from the active resource pack. Missing or
   * malformed entries fall back to the baked-in defaults so generation
   * never fails. Safe to call repeatedly; idempotent for the same
   * resource state.
   */
  ...
  /**
   * Merges the JSON override values into the supplied base entry. Returns
   * null if the base is null AND the override does not provide all
   * required fields (defensive — should not happen in practice since the
   * defaults always seed every category).
   */
  @Nullable
  ...
  private static Integer parseColor(@NotNull JsonObject obj, @NotNull String key) {
    ...
    try {
      ...
      // We only care about the RGB channels; alpha is recomposed at draw time.
