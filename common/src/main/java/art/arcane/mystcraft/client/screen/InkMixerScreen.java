...
public class InkMixerScreen extends AbstractContainerScreen<InkMixerMenu> {
  ...
  private static void addAffinitySection(List<Component> out, InkBlend blend) {
    ...
    for (Map.Entry<ResourceLocation, Float> e : blend.symbolWeights().entrySet()) {
    ...
    if (menu.hasInk() && !menu.getCarried().isEmpty()) {
      ...
      if (isInBasin(relX, relY)) {
        if (InkEffects.hasEffects(menu.getCarried())) {
          guiGraphics.drawString(this.font, Component.literal("Click to add"), 55, 75, GuiTheme.color("text_ok") & 0x00FFFFFF, false);
        } else {
          guiGraphics.drawString(this.font, Component.literal("No ink effect"), 55, 75, GuiTheme.color("text_warning") & 0x00FFFFFF, false);
        }
      }
    }
  }
