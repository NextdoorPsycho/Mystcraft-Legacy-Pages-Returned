...
/**
 * Common config accessor for Mystcraft settings. Platform modules populate
 * these suppliers during initialization. Each supplier wraps the
 * platform-specific config value (ForgeConfigSpec, JSON, etc.).
 */
public final class MystcraftConfig {
   ...
   * but skip optional flourishes (drop shadows, decorative borders) so the UI
   * stays cheap on potato hardware. Defaults to {@code true}.
   */
  public static Supplier<Boolean> proceduralUiEnabled = () -> true;
  /**
   * When false,
   * {@link art.arcane.mystcraft.client.gui.procedural.BookTextureFactory}
   * renders a uniform leather cover regardless of NBT (cheap escape hatch for
   * resource-pack authors who want every book to look identical, or for
   * low-VRAM clients). Defaults to {@code true}.
   */
   ...
   * {@link
   * art.arcane.mystcraft.client.gui.procedural.symbol.SymbolPageTextureFactory}
   * draws only the parchment background — no glyphs, no motif, no flourish, no
   * halo. Pages stay readable as items but lose content-aware art. Defaults to
   * {@code true}.
   */
   ...
   * {@link
   * art.arcane.mystcraft.client.gui.procedural.symbol.SymbolGlyphFactory}) is
   * replaced with a letter-based fallback that renders the first two characters
   * of the symbol's registry path in the centre of each tile using
   * {@code Minecraft.getInstance().font}. Useful for accessibility
   * (high-contrast text), low-end clients (cheaper than primitive composition),
   * and procedural-UI debugging. Defaults to {@code true}.
   */
