...
public class CorruptedColumnsPopulator implements IPopulate {
  ...
  public void populate(WorldGenLevel world, RandomSource random, BlockPos chunkPos) {
    for (int i = 0; i < count; i++) {
      ...
      if (surfaceY <= world.getMinBuildHeight() + 1 || surfaceY >= world.getMaxBuildHeight() - 30) {
        continue;
      }
  ...
  private void generateColumnCluster(WorldGenLevel world, RandomSource random, BlockPos chunkPos,
    ...
    for (int c = 0; c < columnCount; c++) {
      ...
      if (columnSurfaceY[c] <= world.getMinBuildHeight() + 1 || columnSurfaceY[c] >= world.getMaxBuildHeight() - 30) {
        continue;
      }
