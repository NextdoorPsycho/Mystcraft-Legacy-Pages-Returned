...
/**
 * Fluid tank display element for the Mystcraft GUI system (1.19.2 version).
 */
public class MystGuiFluidTank extends MystGuiElement {
  ...
  protected void doRenderBackground(PoseStack poseStack, float partialTick, int mouseX, int mouseY) {
    ...
    GuiComponent.fill(poseStack, x, y, x + width, y + height, 0xFF202020);
    GuiComponent.fill(poseStack, x + 1, y + 1, x + width - 1, y + height - 1, 0xFF101010);
    ...
    if (capacity > 0 && amount > 0) {
      ...
      GuiComponent.fill(poseStack, x + 1, fillY, x + width - 1, y + height - 1, fluidColor);
    }

    // Draw tank border
    GuiComponent.fill(poseStack, x, y, x + width, y + 1, 0xFF404040);
    GuiComponent.fill(poseStack, x, y + height - 1, x + width, y + height, 0xFF404040);
    GuiComponent.fill(poseStack, x, y, x + 1, y + height, 0xFF404040);
    GuiComponent.fill(poseStack, x + width - 1, y, x + width, y + height, 0xFF404040);
  }
  ...
  protected List<Component> getOwnTooltip(int mouseX, int mouseY) {
    int amount = amountProvider.get();
    int capacity = capacityProvider.get();
    return List.of(new TextComponent(fluidName + ": " + amount + "/" + capacity + " mB"));
  }
}
