...
public class MystGuiSurfaceTabs extends MystGuiElement {
  ...
  protected void doRenderBackground(PoseStack poseStack, float partialTick, int mouseX, int mouseY) {
    ...
    GuiComponent.drawCenteredString(poseStack, mc.font, new TextComponent("^"), guiLeft + TAB_WIDTH / 2, tabY + 1, upArrowColor);
    tabY += ARROW_HEIGHT;
    ...
    for (int i = 0; i < TAB_COUNT; i++) {
      ...
      if (!stack.isEmpty()) {
        ...
        // Render item icon
        mc.getItemRenderer().renderAndDecorateItem(stack, guiLeft + TAB_WIDTH - 20, tabY + 8);
      }
    ...
    GuiComponent.fill(poseStack, guiLeft, tabY, guiLeft + TAB_WIDTH, tabY + ARROW_HEIGHT, 0xFF303030);
    GuiComponent.drawCenteredString(poseStack, mc.font, new TextComponent("v"), guiLeft + TAB_WIDTH / 2, tabY + 1, downArrowColor);
  }
