...
/**
 * Text field GUI element for the Mystcraft GUI system (1.19.2 version).
 */
public class MystGuiTextField extends MystGuiElement {
  ...
  protected void doRenderBackground(PoseStack poseStack, float partialTick, int mouseX, int mouseY) {
    ...
    GuiComponent.fill(poseStack, x - 1, y - 1, x + width + 1, y + height + 1, 0xFF000000 | borderColorToUse);

    // Draw background
    GuiComponent.fill(poseStack, x, y, x + width, y + height, 0xFF000000);
    ...
    // Enable scissor
    enableScissor(x + 2, y, x + width - 2, y + height);
    mc.font.draw(poseStack, displayText, textX - scrollOffset, textY, textColor);

    // Draw cursor
    if (focused && editable && (cursorCounter / 6) % 2 == 0) {
      int cursorXPos = textX + mc.font.width(text.substring(0, cursorPosition)) - scrollOffset;
      GuiComponent.fill(poseStack, cursorXPos, textY - 1, cursorXPos + 1, textY + 9, 0xFFD0D0D0);
    }
    ...
    if (selectionEnd != cursorPosition) {
      ...
      GuiComponent.fill(poseStack, selStartX, textY - 1, selEndX, textY + 9, 0x803030FF);
    }
  ...
  private void enableScissor(int x1, int y1, int x2, int y2) {
    ...
    int scissorY = (int) ((mc.getWindow().getGuiScaledHeight() - y2) * scale);
    int scissorWidth = (int) ((x2 - x1) * scale);
    int scissorHeight = (int) ((y2 - y1) * scale);
