...
/**
 * Toggle button GUI element for the Mystcraft GUI system (1.19.2 version).
 */
public class MystGuiToggleButton extends MystGuiElement {
  ...
  protected void doRenderBackground(PoseStack poseStack, float partialTick, int mouseX, int mouseY) {
    ...
    GuiComponent.fill(poseStack, x + 1, y + 1, x + width - 1, y + height - 1, bgColor);
    ...
    if (!text.isEmpty()) {
      ...
      mc.font.draw(poseStack, text, textX, textY, textColor);
    }
  }
