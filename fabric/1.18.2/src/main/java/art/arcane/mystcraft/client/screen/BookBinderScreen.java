...
public class BookBinderScreen extends AbstractContainerScreen<BookBinderMenu> {
  ...
  protected void init() {
    ...
    titleField = new EditBox(this.font, this.leftPos + TEXT_FIELD_X, this.topPos + TEXT_FIELD_Y,
        TEXT_FIELD_WIDTH, TEXT_FIELD_HEIGHT, new TextComponent("Book Title"));
  ...
  protected void renderBg(@NotNull PoseStack poseStack, float partialTick, int mouseX, int mouseY) {
    ...
    // Draw page list
    renderPageList(poseStack, mouseX, mouseY);

    // Draw missing link panel warning if needed
    renderMissingPanelWarning(poseStack, mouseX, mouseY);
  }

  private void renderPageList(PoseStack poseStack, int mouseX, int mouseY) {
    ...
    for (int i = 0; i < maxVisibleSlots; i++) {
      ...
      // Top edge (dark)
      fill(poseStack, x, y, x + PAGE_SIZE + 2, y + 1, slotBorderDark);
      // Left edge (dark)
      fill(poseStack, x, y, x + 1, y + PAGE_SIZE + 2, slotBorderDark);
      // Bottom edge (light)
      fill(poseStack, x, y + PAGE_SIZE + 1, x + PAGE_SIZE + 2, y + PAGE_SIZE + 2, slotBorderLight);
      // Right edge (light)
      fill(poseStack, x + PAGE_SIZE + 1, y, x + PAGE_SIZE + 2, y + PAGE_SIZE + 2, slotBorderLight);
      // Inner background
      fill(poseStack, x + 1, y + 1, x + PAGE_SIZE + 1, y + PAGE_SIZE + 1, slotBg);
    }
    ...
    for (int i = scrollOffset; i < pages.size() && i < scrollOffset + maxVisibleSlots; i++) {
      ...
      if (!page.isEmpty()) {
        // Draw page item
        this.itemRenderer.renderAndDecorateItem(page, x, y);
        ...
        if (i == 0 && !Page.isLinkPanel(page)) {
          ...
          int bx = x - 1;
          int by = y - 1;
          fill(poseStack, bx - 1, by - 1, bx + PAGE_SIZE + 2, by, 0xFFFF0000);
          fill(poseStack, bx - 1, by + PAGE_SIZE + 1, bx + PAGE_SIZE + 2, by + PAGE_SIZE + 2, 0xFFFF0000);
          fill(poseStack, bx - 1, by, bx, by + PAGE_SIZE + 1, 0xFFFF0000);
          fill(poseStack, bx + PAGE_SIZE + 1, by, bx + PAGE_SIZE + 2, by + PAGE_SIZE + 1, 0xFFFF0000);
        }
      }
    }
  }

  private void renderMissingPanelWarning(PoseStack poseStack, int mouseX, int mouseY) {
    List<ItemStack> pages = menu.getBlockEntity().getPageList();
    ...
    RenderSystem.setShaderTexture(0, TEXTURE);
    blit(poseStack, warningX, warningY, 176, 0, 18, 18);
    RenderSystem.setShaderColor(1.0f, 1.0f, 1.0f, 1.0f);
  ...
  public void render(@NotNull PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
    ...
    renderWarningTooltip(poseStack, mouseX, mouseY);
  }

  private void renderWarningTooltip(PoseStack poseStack, int mouseX, int mouseY) {
    List<ItemStack> pages = menu.getBlockEntity().getPageList();
    boolean showWarning = pages.isEmpty() || !Page.isLinkPanel(pages.get(0));
    if (!showWarning) {
      return;
    ...
    if (mouseX >= warningX && mouseX < warningX + 18 &&
      ...
      renderTooltip(poseStack,
          List.of(
              new TextComponent("Missing Link Panel"),
              new TextComponent("Add a link panel as the first page")
          ),
  ...
  protected void renderLabels(@NotNull PoseStack poseStack, int mouseX, int mouseY) {
    ...
    this.font.draw(poseStack, pageText, this.imageWidth - 8 - this.font.width(pageText), 9, 4210752);
  }
