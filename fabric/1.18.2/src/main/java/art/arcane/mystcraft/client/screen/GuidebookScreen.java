...
/**
 * Screen for displaying the Mystcraft Guidebook (1.19.2 version).
...
public class GuidebookScreen extends Screen {
  ...
  public GuidebookScreen() {
    super(new TranslatableComponent("item.mystcraft.guidebook"));
  ...
  public void render(@NotNull PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
    ...
    blit(poseStack, leftPos, topPos, 0, 0, BOOK_WIDTH, BOOK_HEIGHT, 512, 256);

    if (showingTableOfContents) {
      renderTableOfContents(poseStack, mouseX, mouseY);
    } else {
      renderChapterContent(poseStack, mouseX, mouseY);
    }

    // Don't call super.render - we're not using widgets
  }

  private void renderTableOfContents(PoseStack poseStack, int mouseX, int mouseY) {
    // Left page - Title and description (positioned higher)
    int leftX = leftPos + LEFT_PAGE_X;
    int y = topPos + 10;

    // Book title - bold
    Component title = new TextComponent("Rehevkor").withStyle(ChatFormatting.BOLD);
    int titleWidth = font.width(title);
    font.draw(poseStack, title, leftX + (LEFT_PAGE_WIDTH - titleWidth) / 2, y, TITLE_COLOR);
    y += LINE_HEIGHT + 2;

    // Subtitle - italic
    Component subtitle = new TextComponent("The Art of Writing").withStyle(ChatFormatting.ITALIC);
    int subtitleWidth = font.width(subtitle);
    font.draw(poseStack, subtitle, leftX + (LEFT_PAGE_WIDTH - subtitleWidth) / 2, y, TEXT_COLOR);
    y += LINE_HEIGHT + 6;

    // Decorative line
    int lineStart = leftX + 12;
    int lineEnd = leftX + LEFT_PAGE_WIDTH - 12;
    fill(poseStack, lineStart, y, lineEnd, y + 1, 0xFF000000 | TITLE_COLOR);
    y += 10;
    ...
    for (String line : desc) {
      if (!line.isEmpty()) {
        font.draw(poseStack, line, leftX + (LEFT_PAGE_WIDTH - font.width(line)) / 2, y, TEXT_COLOR);
      }
      y += LINE_HEIGHT;
    }
    ...
    // Chapters title - bold
    Component tocTitle = new TextComponent("Chapters").withStyle(ChatFormatting.BOLD);
    int tocWidth = font.width(tocTitle);
    font.draw(poseStack, tocTitle, rightX + (RIGHT_PAGE_WIDTH - tocWidth) / 2, y, TITLE_COLOR);
    y += LINE_HEIGHT + 8;
    ...
    for (int i = 0; i < visibleCount; i++) {
      ...
      Component entry = new TextComponent(entryText).withStyle(ChatFormatting.BOLD);
      font.draw(poseStack, entry, itemX, itemY, color);
    }
    ...
    if (chapters.size() > maxVisible) {
      ...
      font.draw(poseStack, scrollHint, rightX + (RIGHT_PAGE_WIDTH - font.width(scrollHint)) / 2, scrollY, PAGE_NUM_COLOR);
    }
  }

  private void renderChapterContent(PoseStack poseStack, int mouseX, int mouseY) {
    List<GuidebookChapter> chapters = content.getChapters();
    ...
    Component chapterTitle = new TextComponent(chapter.getTitle()).withStyle(ChatFormatting.BOLD);
    font.draw(poseStack, chapterTitle, leftPos + LEFT_PAGE_X, topPos + 6, TITLE_COLOR);

    // Render left page
    if (currentPageIndex < pages.size()) {
      renderPage(poseStack, pages.get(currentPageIndex), leftPos + LEFT_PAGE_X, topPos + TEXT_TOP, LEFT_PAGE_WIDTH);
    }

    // Render right page
    if (currentPageIndex + 1 < pages.size()) {
      renderPage(poseStack, pages.get(currentPageIndex + 1), leftPos + RIGHT_PAGE_X, topPos + TEXT_TOP, RIGHT_PAGE_WIDTH);
    }
    ...
    font.draw(poseStack, leftNum, leftNumX, topPos + TEXT_BOTTOM + 2, PAGE_NUM_COLOR);

    if (currentPageIndex + 1 < pages.size()) {
      String rightNum = "- " + rightPageNum + " -";
      int rightNumX = leftPos + RIGHT_PAGE_X + (RIGHT_PAGE_WIDTH - font.width(rightNum)) / 2;
      font.draw(poseStack, rightNum, rightNumX, topPos + TEXT_BOTTOM + 2, PAGE_NUM_COLOR);
    }
    ...
    if (hasPrev) {
      ...
      int color = hovered ? LINK_HOVER_COLOR : LINK_COLOR;
      font.draw(poseStack, "<< Prev", leftPos + PREV_ARROW_X, topPos + ARROW_Y, color);
    }
    ...
    if (hasNext) {
      ...
      int color = hovered ? LINK_HOVER_COLOR : LINK_COLOR;
      font.draw(poseStack, nextText, leftPos + NEXT_ARROW_X - font.width(nextText), topPos + ARROW_Y, color);
    }
    ...
    font.draw(poseStack, contentsText, contentsX, topPos + ARROW_Y, contentsHovered ? LINK_HOVER_COLOR : LINK_COLOR);
  }

  private void renderPage(PoseStack poseStack, GuidebookPage page, int x, int y, int width) {
    ...
    if (title != null && !title.isEmpty()) {
      Component titleComponent = new TextComponent(title).withStyle(ChatFormatting.BOLD);
      font.draw(poseStack, titleComponent, x, contentY, TITLE_COLOR);
      contentY += LINE_HEIGHT + 2;
      // Subtle underline
      fill(poseStack, x, contentY - 1, x + Math.min(font.width(titleComponent), width), contentY, 0xFF000000 | TITLE_COLOR);
    ...
    List<FormattedCharSequence> lines = this.font.split(new TextComponent(pageContent), width);
    ...
    for (FormattedCharSequence line : lines) {
      ...
      font.draw(poseStack, line, x, contentY + (lineCount * LINE_HEIGHT), color);
      lineCount++;
