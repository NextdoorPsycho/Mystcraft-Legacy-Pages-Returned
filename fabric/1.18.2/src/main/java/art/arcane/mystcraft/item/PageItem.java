  ...
  public Component getName(@NotNull ItemStack stack) {
    if (stack.getTag() != null) {
      if (Page.isLinkPanel(stack)) {
        return new TranslatableComponent("item.mystcraft.page.panel");
      }
      if (Page.isBlank(stack)) {
        return new TranslatableComponent("item.mystcraft.page.blank");
      }
      ResourceLocation symbolId = Page.getSymbol(stack);
      if (symbolId != null) {
        IAgeSymbol symbol = SymbolRegistry.get(symbolId);
        if (symbol != null) {
          return new TranslatableComponent("item.mystcraft.page.symbol", symbol.getLocalizedName());
        }
        return new TranslatableComponent("item.mystcraft.page.symbol", symbolId.getPath());
      }
    }
    return new TranslatableComponent("item.mystcraft.page.blank");
  }
