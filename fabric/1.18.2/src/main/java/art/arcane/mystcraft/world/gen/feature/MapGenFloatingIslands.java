...
public class MapGenFloatingIslands implements ITerrainAlteration {
  ...
  @Override
  public void alterTerrain(ServerLevel world, int chunkX, int chunkZ, ChunkAccess chunk, Random random) {
    for (int dx = -RANGE; dx <= RANGE; dx++) {
      for (int dz = -RANGE; dz <= RANGE; dz++) {
        int originChunkX = chunkX + dx;
        int originChunkZ = chunkZ + dz;

        Random chunkRand = getChunkRandom(originChunkX, originChunkZ);
  ...
  private IslandType pickIslandType(Random rand) {
  ...
  private void generateSmallIslet(ChunkAccess chunk, int chunkX, int chunkZ,
    ...
    Random rand = new Random(isletSeed);
  ...
  private void generateShard(ChunkAccess chunk, int chunkX, int chunkZ, long shardSeed,
                             int centerX, int centerY, int centerZ) {
    Random rand = new Random(shardSeed);
  ...
  private void generateFragment(ChunkAccess chunk, int chunkX, int chunkZ, long debrisSeed,
                                int centerX, int centerY, int centerZ) {
    Random rand = new Random(debrisSeed);
  ...
  protected Random getChunkRandom(int chunkX, int chunkZ) {
