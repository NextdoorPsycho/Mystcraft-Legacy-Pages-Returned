...
/**
 * A scrollable surface that displays pages/symbols (1.19.2 version).
 */
public class MystGuiPageSurface extends MystGuiElement {
  ...
  protected void doRenderBackground(PoseStack poseStack, float partialTick, int mouseX, int mouseY) {
    ...
    // Draw background
    GuiComponent.fill(poseStack, guiLeft, guiTop, guiLeft + contentWidth, guiTop + height, 0xAA000000);

    // Draw scrollbar background
    GuiComponent.fill(poseStack, guiLeft + contentWidth, guiTop, guiLeft + width, guiTop + height, 0x80404040);
    ...
    if (maxScroll > 0) {
      ...
      GuiComponent.fill(poseStack, guiLeft + contentWidth + 2, scrollbarY, guiLeft + width - 2, scrollbarY + scrollbarHeight, 0xFFC0C0C0);
    }
    ...
    if (pages != null) {
      for (PositionableItem item : pages) {
        ...
        if (item.count > 0 && !stack.isEmpty()) {
          renderPage(poseStack, stack, (int) pageX, (int) pageY, (int) PAGE_WIDTH, (int) PAGE_HEIGHT);

          // Show count if more than 1
          if (item.count > 1) {
            String countStr = String.valueOf(item.count);
            mc.font.draw(poseStack, countStr,
          ...
          if (mouseX >= pageX && mouseX < pageX + PAGE_WIDTH &&
            ...
            // Draw highlight
            GuiComponent.fill(poseStack, (int) pageX, (int) pageY,
  ...
  private void renderPage(PoseStack poseStack, ItemStack stack, int x, int y, int width, int height) {
    // Use PageRenderHelper to draw the page with D'ni symbols
    PageRenderHelper.drawPage(poseStack, stack, x, y, width, height, 0);
  }
  ...
  private void updateHoverTooltip(ItemStack stack) {
    ...
    String displayName = getDisplayName(stack);
    if (displayName != null) {
      hoverTooltip.add(new TextComponent(displayName));
    }
  }
