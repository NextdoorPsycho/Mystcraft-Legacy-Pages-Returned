...
/**
 * Renders a rainbow arc in the sky for Ages with the Rainbow symbol enabled (1.18.2).
...
public class RainbowRenderer {
  ...
  // 1.18.2: Use RenderLevelLastEvent instead of RenderLevelStageEvent
  @SubscribeEvent
  public static void onRenderLevelLast(RenderLevelLastEvent event) {
    ...
    renderRainbow(event.getPoseStack(), mc.gameRenderer.getMainCamera(), gameTime, rainbowAlpha);
  }
  ...
  private static void renderRainbow(PoseStack poseStack, Camera camera, float gameTime, float alpha) {
    ...
    // 1.18.2: Use Vector3f.YP/XP.rotationDegrees() instead of Axis
    poseStack.mulPose(Vector3f.YP.rotationDegrees(-rainbowAngle));

    // Slight tilt based on sun height
    float tilt = 20.0f + Mth.sin(dayTime * (float) Math.PI * 2.0f) * 10.0f;
    poseStack.mulPose(Vector3f.XP.rotationDegrees(tilt));

    // 1.18.2: Use com.mojang.math.Matrix4f
