...
/**
 * Screen for viewing book contents (Agebooks and Linkbooks) - 1.19.2 version.
...
public class BookScreen extends Screen {
  ...
  private void performLink() {
    if (blockPos != null) {
      ...
      ForgeMystcraftNetwork_1_18_2.sendToServer(new BlockBookActivatePacket(blockPos));
    } else if (entityId == HAND_BASED) {
      // Book is in player's hand
      ForgeMystcraftNetwork_1_18_2.sendToServer(new LinkBookActivatePacket(hand));
    } else {
      // Book is on a LinkbookEntity
      ForgeMystcraftNetwork_1_18_2.sendToServer(new EntityBookActivatePacket(entityId));
    }
    this.onClose();
  }
