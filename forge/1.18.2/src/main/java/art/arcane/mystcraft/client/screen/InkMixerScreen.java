...
public class InkMixerScreen extends AbstractContainerScreen<InkMixerMenu> {
  ...
  protected void renderBg(@NotNull PoseStack poseStack, float partialTick, int mouseX, int mouseY) {
    ...
    blit(poseStack, x + BASIN_LEFT, y + BASIN_TOP, 179, 16, BASIN_WIDTH, BASIN_HEIGHT);

    // Draw ink fill if mixer has ink
    if (menu.hasInk()) {
      renderInkTank(poseStack, x + BASIN_LEFT, y + BASIN_TOP, BASIN_WIDTH, BASIN_HEIGHT);
    }
   ...
   */
  private void renderInkTank(PoseStack poseStack, int left, int top, int width, int height) {
    // Base ink color (dark blue/black)
    int baseColor = 0xFF101030;
    fill(poseStack, left, top, left + width, top + height, baseColor);
    ...
    if (probabilities != null && !probabilities.isEmpty()) {
      ...
      if (totalWeight > 0) {
        ...
        fillGradient(poseStack, left, top, left + width, top + height, topColor, bottomColor);

        // Draw animated color ball in center
        renderColorBall(poseStack, left + width / 2, top + height / 2, 20, totalR, totalG, totalB, pulse);
      }
    }
  }
   ...
   */
  private void renderColorBall(PoseStack poseStack, int centerX, int centerY, float radius,
  ...
  public void render(@NotNull PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
    ...
    if (menu.hasInk()) {
      ...
      if (isInBasin(relX, relY)) {
        renderInkTooltip(poseStack, mouseX, mouseY);
      }
    }
  }
   ...
   */
  private void renderInkTooltip(PoseStack poseStack, int mouseX, int mouseY) {
    Map<String, Float> probabilities = menu.getInkProbabilities();
    if (probabilities == null || probabilities.isEmpty()) {
      return;
    }
    ...
    tooltip.add(new TextComponent("Ink Properties:"));
    ...
    for (Map.Entry<String, Float> entry : probabilities.entrySet()) {
      ...
      tooltip.add(new TextComponent("  " + name + ": " + percent + "%")
    ...
    if (menu.hasInk() && !menu.getCarried().isEmpty()) {
      ...
      if (isInBasin(relX, relY)) {
        if (InkEffects.hasEffects(menu.getCarried())) {
          this.font.draw(poseStack, new TextComponent("Click to add"), 55, 75, 0x40FF40);
        } else {
          this.font.draw(poseStack, new TextComponent("No ink effect"), 55, 75, 0xFF4040);
        }
      }
    }
  }
