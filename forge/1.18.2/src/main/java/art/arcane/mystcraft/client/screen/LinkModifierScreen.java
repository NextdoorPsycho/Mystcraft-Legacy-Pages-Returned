...
/**
 * Screen for the Link Modifier block (1.19.2 version).
 */
public class LinkModifierScreen extends AbstractContainerScreen<LinkModifierMenu> {
  ...
  private MystGuiToggleButton createFlagButton(String property, int x, int y, int size) {
    return new MystGuiToggleButton(
        ...
        property,
        List.of(new TextComponent(InkEffects.getLocalizedName(property))),
        x, y, size, size
    );
  }
  ...
  protected void renderBg(@NotNull PoseStack poseStack, float partialTick, int mouseX, int mouseY) {
    ...
    if (hasDeadLink()) {
      renderDeadLinkWarning(poseStack);
    }
  }
  ...
  public void render(@NotNull PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
    ...
    if (hasDeadLink()) {
      ...
      if (mouseX >= bookSlotX && mouseX < bookSlotX + 16 &&
        ...
        renderDeadLinkTooltip(poseStack, mouseX, mouseY);
      }
    }
  }
  ...
  protected void renderLabels(@NotNull PoseStack poseStack, int mouseX, int mouseY) {
    ...
    if (hasDeadLink()) {
      ...
      this.font.draw(poseStack, warning, (this.imageWidth - textWidth) / 2, 55, color);
    }
  }
   ...
   */
  private void renderDeadLinkWarning(PoseStack poseStack) {
    float pulse = (float) (0.5 + 0.5 * Math.sin(warningPulse * 0.15));
    int color = lerpColor(WARNING_COLOR_DARK, WARNING_COLOR_LIGHT, pulse);
    ...
    // Top border
    fill(poseStack, slotX - 1, slotY - 1, slotX + slotSize + 1, slotY, color);
    // Bottom border
    fill(poseStack, slotX - 1, slotY + slotSize, slotX + slotSize + 1, slotY + slotSize + 1, color);
    // Left border
    fill(poseStack, slotX - 1, slotY, slotX, slotY + slotSize, color);
    // Right border
    fill(poseStack, slotX + slotSize, slotY, slotX + slotSize + 1, slotY + slotSize, color);
  }

  /**
   * Renders tooltip explaining the dead link.
   */
  private void renderDeadLinkTooltip(PoseStack poseStack, int mouseX, int mouseY) {
    List<Component> tooltip = new ArrayList<>();
    tooltip.add(new TextComponent("Dead Link").withStyle(style -> style.withColor(0xFF4444)));
    tooltip.add(new TextComponent("The linked Age no longer exists."));
    tooltip.add(new TextComponent(""));
    tooltip.add(new TextComponent("This book cannot be used to link."));
    tooltip.add(new TextComponent("Consider recycling it for ink."));
