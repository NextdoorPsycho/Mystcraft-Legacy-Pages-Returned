 ...
 * <p>
 * 1.18.2 version using TextComponent instead of Component.literal().
 */
    ...
    if (isLinkPanel(page)) {
      ...
      if (properties != null && !properties.isEmpty()) {
        for (String property : properties) {
          ...
          if (color != null) {
            // Add colored text (1.18.2 version using TextComponent)
            int rgb = color.toRGB();
            list.add(new TextComponent(localizedName).withStyle(style ->
                style.withColor(rgb)));
          } else {
            list.add(new TextComponent(localizedName));
          }
        }
      }
    }
