...
/**
 * The Ink Vial item - 1.18.2 version.
...
public class InkVialItem extends Item {
  ...
  public void appendHoverText(@NotNull ItemStack stack, @Nullable Level level, @NotNull List<Component> tooltip, @NotNull TooltipFlag flag) {
    ...
    if (amount > 0) {
      tooltip.add(new TranslatableComponent("item.mystcraft.inkvial.amount", amount, MAX_INK));
      String colorHex = String.format("#%06X", color);
      tooltip.add(new TranslatableComponent("item.mystcraft.inkvial.color", colorHex));
    } else {
      tooltip.add(new TranslatableComponent("item.mystcraft.inkvial.empty"));
    }
  }

  // 1.18.2: Durability bar methods don't exist - use damage-based display instead
  // The bar is controlled by isDamaged/getDamageValue in 1.18.2
