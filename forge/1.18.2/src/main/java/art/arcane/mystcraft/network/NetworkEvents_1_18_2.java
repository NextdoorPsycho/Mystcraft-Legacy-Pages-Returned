...
/**
 * Event handlers for network-related events (1.18.2 version).
 * Handles syncing data to players when they join.
 */
@Mod.EventBusSubscriber(modid = Mystcraft.MOD_ID)
public final class NetworkEvents_1_18_2 {

  private NetworkEvents_1_18_2() {
  }
  ...
  public static void onPlayerLogin(PlayerEvent.PlayerLoggedInEvent event) {
    if (event.getEntity() instanceof ServerPlayer player) {
      ...
      Mystcraft.LOGGER.debug("Player {} logged in (symbol sync disabled in 1.18.2)", player.getName().getString());
    }
  }
