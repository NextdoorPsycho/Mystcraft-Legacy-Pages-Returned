...
public class AgeDimensionFactory {
   ...
   * Returns the Age's configured spawn or a default based on the world spawn.
   */
  public static net.minecraft.core.BlockPos getAgeSpawn(ServerLevel level) {
    AgeData ageData = AgeData.getIfPresent(level);
    if (ageData != null) {
      int x = ageData.getSpawnX();
      int y = ageData.getSpawnY();
      int z = ageData.getSpawnZ();
      // Only use Age spawn if it's been set (non-zero Y)
      if (y > 0) {
