...
public class VerticalTendrilsPopulator implements IPopulate {
  ...
  private void generateTendril(WorldGenLevel world, long pathSeed, long decorSeed,
    ...
    if (fromCeiling) {
      ...
      if (!world.getBlockState(checkPos).getMaterial().isSolid() || !world.getBlockState(checkPos.below()).isAir()) {
        return;
      }
      ...
      if (!ground.getMaterial().isSolid() || ground.is(BlockTags.LEAVES)) {
        return;
      }
  ...
  private BlockState getDecorationBlock(BlockState baseBlock, Random random) {
    if (random.nextInt(3) == 0) {
      ...
      return switch (choice) {
        ...
        case 1 -> Blocks.VINE.defaultBlockState(); // 1.18.2: SCULK_VEIN doesn't exist
        case 2 -> Blocks.SHROOMLIGHT.defaultBlockState();
        case 3 -> Blocks.SOUL_LANTERN.defaultBlockState();
        case 4 -> Blocks.SEA_LANTERN.defaultBlockState(); // 1.18.2: OCHRE_FROGLIGHT doesn't exist
