...
/**
 * Base class for Mystcraft GUI elements (1.19.2 version).
 */
public abstract class MystGuiElement {
  ...
  public void renderBackground(PoseStack poseStack, float partialTick, int mouseX, int mouseY) {
    if (!visible) return;
    doRenderBackground(poseStack, partialTick, mouseX, mouseY);
    for (MystGuiElement child : children) {
      child.renderBackground(poseStack, partialTick, mouseX, mouseY);
    }
  }
  ...
  public void renderForeground(PoseStack poseStack, int mouseX, int mouseY) {
    if (!visible) return;
    doRenderForeground(poseStack, mouseX, mouseY);
    for (MystGuiElement child : children) {
      child.renderForeground(poseStack, mouseX, mouseY);
    }
  }

  protected void doRenderForeground(PoseStack poseStack, int mouseX, int mouseY) {
  }
