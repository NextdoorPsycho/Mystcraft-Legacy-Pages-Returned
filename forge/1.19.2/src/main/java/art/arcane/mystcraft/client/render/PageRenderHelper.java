...
/**
 * Helper class for rendering pages with D'ni symbols (1.19.2 version using PoseStack).
 */
public class PageRenderHelper {
   ...
   */
  public static void drawPage(PoseStack poseStack, ItemStack page, float x, float y,
                              float width, float height, float zLevel) {
    // Draw the page background
    drawPageBackground(poseStack, page, x, y, width, height, zLevel);

    // Draw the symbol or link panel
    ResourceLocation symbolId = Page.getSymbol(page);
    if (symbolId != null) {
      IAgeSymbol symbol = SymbolRegistry.get(symbolId);
      drawSymbol(poseStack, symbol, width - 1, x + 0.5f, y + (height + 1 - width) / 2, zLevel);
    } else if (Page.isLinkPanel(page)) {
      // Draw black rectangle for link panel
      drawFilledRect(poseStack,
   ...
   */
  private static void drawPageBackground(PoseStack poseStack, ItemStack page,
    ...
    drawTexturedRect(poseStack, x, y, width, height,
   ...
   */
  public static void drawSymbol(PoseStack poseStack, IAgeSymbol symbol, float scale,
    ...
    if (symbol == null) {
      // Draw unknown symbol (?)
      drawWord(poseStack, null, scale, x, y, zLevel);
      return;
    }
    ...
    if (words == null || words.length == 0) {
      ...
      drawWord(poseStack, DrawableWordManager.getDrawableWord(null), s * 2, x + o, y + o, zLevel);
      return;
    }
    ...
    // Position 3: left
    if (words.length > 0)
      drawWord(poseStack, DrawableWordManager.getDrawableWord(words[0]), 2 * s, x + o, y, zLevel);
    if (words.length > 1)
      drawWord(poseStack, DrawableWordManager.getDrawableWord(words[1]), 2 * s, x + o * 2, y + o, zLevel);
    if (words.length > 2)
      drawWord(poseStack, DrawableWordManager.getDrawableWord(words[2]), 2 * s, x + o, y + o * 2, zLevel);
    if (words.length > 3)
      drawWord(poseStack, DrawableWordManager.getDrawableWord(words[3]), 2 * s, x, y + o, zLevel);
   ...
   */
  public static void drawWord(PoseStack poseStack, DrawableWord word, float scale,
                              float x, float y, float zLevel) {
    ...
    for (int i = 0; i < components.size(); i++) {
      ...
      drawComponent(poseStack, components.get(i), color, scale, x, y, zLevel);
    }
  }
   ...
   */
  private static void drawFilledRect(PoseStack poseStack, float x1, float y1, float x2, float y2,
