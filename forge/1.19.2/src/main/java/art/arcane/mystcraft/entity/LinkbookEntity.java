...
public class LinkbookEntity extends Entity {
  ...
  public void activateBook(Entity entity) {
    ...
    if (book.getItem() instanceof LinkbookItem linkbook) {
      linkbook.activate(book, getLevel(), entity);
    } else if (book.getItem() instanceof AgebookItem agebook) {
      agebook.activate(book, getLevel(), entity);
    }
  }
  ...
  public void tick() {
    ...
    if (!getLevel().isClientSide && !getBookItem().isEmpty()) {
  ...
  private void processEnvironmentalDamage() {
    ...
    FluidState fluidState = getLevel().getFluidState(blockPosition());
    ...
    if (ticksExisted % 20 == 0) { // Check every second
      ...
      List<Entity> entities = getLevel().getEntities(this, box, e -> e != this && !(e instanceof Player));
      if (!entities.isEmpty()) {
  ...
  private void processGroundDecay() {
    ...
    if (isOnGround() && getLevel().getFluidState(blockPosition()).isEmpty()) {
      ticksOnGround++;
  ...
  private void destroyBook() {
    if (!getLevel().isClientSide) {
      ItemStack page = art.arcane.mystcraft.registry.ModItems.PAGE != null
          ...
          : ItemStack.EMPTY;
      if (!page.isEmpty()) {
        getLevel().addFreshEntity(new net.minecraft.world.entity.item.ItemEntity(getLevel(), getX(), getY(), getZ(), page));
      }
      getLevel().addFreshEntity(new net.minecraft.world.entity.item.ItemEntity(getLevel(), getX(), getY(), getZ(), new ItemStack(net.minecraft.world.item.Items.LEATHER)));
    }
    discard();
  }
