...
public interface IPopulate {
  ...
  default boolean isInWritableArea(BlockPos pos, BlockPos chunkPos) {
    return isInWritableColumn(pos.getX(), pos.getZ(), chunkPos);
  }

  /**
   * Checks a horizontal column before reading a heightmap or block state. Reads
   * outside the feature-stage write radius are unsafe even when no block is
   * ultimately placed there.
   *
   * @param x        The world X coordinate
   * @param z        The world Z coordinate
   * @param chunkPos The chunk origin (block coordinates of chunk corner)
   * @return true if the column is available to this population pass
   */
  default boolean isInWritableColumn(int x, int z, BlockPos chunkPos) {
    int chunkMinX = chunkPos.getX();
    int chunkMinZ = chunkPos.getZ();

    return x >= chunkMinX - 16 && x < chunkMinX + 32
        && z >= chunkMinZ - 16 && z < chunkMinZ + 32;
  }
