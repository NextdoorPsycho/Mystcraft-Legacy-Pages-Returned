...
public class MystGuiSurfaceTabs extends MystGuiElement {
  ...
  protected void doRenderBackground(GuiGraphicsExtractor graphics, float partialTick, int mouseX, int mouseY) {
    ...
    for (int i = 0; i < TAB_COUNT; i++) {
      ...
      graphics.text(mc.font, String.valueOf(slot), guiLeft + 4, tabY + 3, textOn);
      ...
      if (!stack.isEmpty()) {
        ...
        graphics.text(mc.font, name, guiLeft + 4, tabY + TAB_HEIGHT - 12, textOff);

        graphics.item(stack, guiLeft + TAB_WIDTH - 20, tabY + 8);
      }
