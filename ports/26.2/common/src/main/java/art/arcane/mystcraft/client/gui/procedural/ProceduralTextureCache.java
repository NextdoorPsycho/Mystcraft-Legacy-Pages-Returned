...
public final class ProceduralTextureCache {
  ...
  private final LinkedHashMap<String, Identifier> cache;
  ...
  private static void releaseTexture(Identifier loc) {
    try {
   ...
   * <p>
   * If the key is already cached the existing {@link Identifier} is
   ...
   * @return the {@link Identifier} that can be passed to
   * {@link net.minecraft.client.gui.GuiGraphicsExtractorExtractor#blit}
   */
  public synchronized Identifier getOrCreate(@NotNull String key, int width, int height,
    ...
    DynamicTexture texture = new DynamicTexture(
        () -> "Mystcraft procedural texture " + safeKey,
        image);
    Identifier loc = Identifier.fromNamespaceAndPath(NAMESPACE, safeKey);
  ...
  public synchronized void clear() {
    for (Iterator<Map.Entry<String, Identifier>> it = cache.entrySet().iterator(); it.hasNext(); ) {
  ...
  public synchronized void invalidate(@NotNull String key) {
    Identifier removed = cache.remove(key);
    if (removed != null) {
