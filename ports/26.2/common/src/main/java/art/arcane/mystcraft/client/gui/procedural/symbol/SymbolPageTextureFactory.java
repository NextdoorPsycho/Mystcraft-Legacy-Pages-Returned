...
public final class SymbolPageTextureFactory {
   ...
   */
  @NotNull
  public static Identifier getPageTexture(@NotNull ItemStack stack) {
  ...
  private static PageInfo analyse(@NotNull ItemStack stack) {
    ...
    Identifier sym = Page.getSymbol(stack);
    if (sym != null) {
  ...
  private static void drawSymbolMotif(@NotNull NativeImage image, @NotNull Identifier symbolId,
  ...
  private static int readPixel(NativeImage image, int x, int y) {
    if (x < 0 || y < 0 || x >= image.getWidth() || y >= image.getHeight())
      return 0;
    int abgr = net.minecraft.util.ARGB.toABGR(image.getPixel(x, y));
    int a = (abgr >>> 24) & 0xFF;
    int b = (abgr >>> 16) & 0xFF;
    int g = (abgr >>> 8) & 0xFF;
    int r = abgr & 0xFF;
