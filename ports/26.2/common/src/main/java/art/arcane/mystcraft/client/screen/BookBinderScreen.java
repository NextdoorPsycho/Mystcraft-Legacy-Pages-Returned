...
public class BookBinderScreen extends AbstractContainerScreen<BookBinderMenu> {
  ...
  private void renderPageList(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
  ...
  private void renderMissingPanelWarning(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
    List<ItemStack> pages = menu.getBlockEntity().getPageList();
    ...
    ProceduralUI.drawWarningGlyph(guiGraphics, warningX, warningY, 18, 18, warningAlpha);
  }
  ...
  private void renderWarningTooltip(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
    List<ItemStack> pages = menu.getBlockEntity().getPageList();
    boolean showWarning = pages.isEmpty() || !Page.isLinkPanel(pages.get(0));
    if (!showWarning) {
      return;
  ...
  protected void extractLabels(@NotNull GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
    ...
    guiGraphics.text(this.font, pageText, this.imageWidth - 8 - this.font.width(pageText), 12, textColor, false);
  }
