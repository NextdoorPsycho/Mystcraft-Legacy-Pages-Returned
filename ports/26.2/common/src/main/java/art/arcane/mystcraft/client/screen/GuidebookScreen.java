...
public class GuidebookScreen extends Screen {
  ...
  public void extractRenderState(@NotNull GuiGraphicsExtractor graphics, int mouseX, int mouseY,
    ...
    Identifier texture = GuidebookTexture.getTexture();
    graphics.blit(RenderPipelines.GUI_TEXTURED, texture, leftPos, topPos, 0, 0,
        BOOK_WIDTH, BOOK_HEIGHT, 512, 256);
  ...
  private void renderTableOfContents(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
    ...
    graphics.text(this.font, title, leftX + (LEFT_PAGE_WIDTH - titleWidth) / 2, y, TITLE_COLOR, false);
    y += LINE_HEIGHT + 2;
    ...
    graphics.text(this.font, subtitle, leftX + (LEFT_PAGE_WIDTH - subtitleWidth) / 2, y, TEXT_COLOR, false);
    y += LINE_HEIGHT + 6;
    ...
    for (String line : desc) {
      if (!line.isEmpty()) {
        graphics.text(this.font, line, leftX + (LEFT_PAGE_WIDTH - font.width(line)) / 2, y, TEXT_COLOR, false);
      }
      y += LINE_HEIGHT;
    }
    ...
    graphics.text(this.font, tocTitle, rightX + (RIGHT_PAGE_WIDTH - tocWidth) / 2, y, TITLE_COLOR, false);
    y += LINE_HEIGHT + 8;
    ...
    for (int i = 0; i < visibleCount; i++) {
      ...
      graphics.text(this.font, entry, itemX, itemY, color, false);
    }

    if (chapters.size() > maxVisible) {
      int scrollY = y + (visibleCount * itemHeight) + 4;
      String scrollHint = "(scroll for more)";
      graphics.text(this.font, scrollHint, rightX + (RIGHT_PAGE_WIDTH - font.width(scrollHint)) / 2, scrollY, PAGE_NUM_COLOR, false);
    }
  }

  private void renderChapterContent(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
    List<GuidebookChapter> chapters = content.getChapters();
    ...
    graphics.text(this.font, chapterTitle, leftPos + LEFT_PAGE_X, topPos + 6, TITLE_COLOR, false);
    ...
    graphics.text(this.font, leftNum, leftNumX, topPos + TEXT_BOTTOM + 2, PAGE_NUM_COLOR, false);

    if (currentPageIndex + 1 < pages.size()) {
      String rightNum = "- " + rightPageNum + " -";
      int rightNumX = leftPos + RIGHT_PAGE_X + (RIGHT_PAGE_WIDTH - font.width(rightNum)) / 2;
      graphics.text(this.font, rightNum, rightNumX, topPos + TEXT_BOTTOM + 2, PAGE_NUM_COLOR, false);
    }
    ...
    if (hasPrev) {
      ...
      int color = hovered ? LINK_HOVER_COLOR : LINK_COLOR;
      graphics.text(this.font, "<< Prev", leftPos + PREV_ARROW_X, topPos + ARROW_Y, color, false);
    }
    ...
    if (hasNext) {
      ...
      int color = hovered ? LINK_HOVER_COLOR : LINK_COLOR;
      graphics.text(this.font, nextText, leftPos + NEXT_ARROW_X - font.width(nextText), topPos + ARROW_Y, color, false);
    }
    ...
    graphics.text(this.font, contentsText, contentsX, topPos + ARROW_Y, contentsHovered ? LINK_HOVER_COLOR : LINK_COLOR, false);
  }

  private void renderPage(GuiGraphicsExtractor graphics, GuidebookPage page, int x, int y, int width) {
    ...
    if (title != null && !title.isEmpty()) {
      ...
      graphics.text(this.font, titleComponent, x, contentY, TITLE_COLOR, false);
    ...
    for (FormattedCharSequence line : lines) {
      ...
      graphics.text(this.font, line, x, contentY + (lineCount * LINE_HEIGHT), color, false);
      lineCount++;
