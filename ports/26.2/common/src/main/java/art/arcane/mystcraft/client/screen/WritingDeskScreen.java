...
public class WritingDeskScreen extends AbstractContainerScreen<WritingDeskMenu> {
  ...
  private IAgeSymbol hoveredSymbol;
  ...
  public WritingDeskScreen(WritingDeskMenu menu, Inventory playerInventory, Component title) {
    super(menu, playerInventory, title, TOTAL_WIDTH, TOTAL_HEIGHT);
  }
  ...
  private void renderSlotBackgrounds(GuiGraphicsExtractor guiGraphics) {
  ...
  private void renderBookPageListBorder(GuiGraphicsExtractor guiGraphics) {
  ...
  public void extractRenderState(@NotNull GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY,
    ...
    hoveredSymbol = getHoveredSymbolFromProvider(mouseX, mouseY);
    if (hoveredSymbol != null) {
      renderSymbolCostIndicator(guiGraphics, mouseX, mouseY);
    }
  }
    ...
    for (MystGuiPageSurface.PositionableItem item : pages) {
      ...
      if (mouseX >= itemX && mouseX < itemX + itemWidth &&
        ...
        if (!item.itemstack.isEmpty()) {
          Identifier symbolId = Page.getSymbol(item.itemstack);
          if (symbolId != null) {
  ...
  private void renderSymbolCostIndicator(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
    ...
    guiGraphics.setTooltipForNextFrame(this.font, costTooltip, java.util.Optional.empty(), mouseX + 16, mouseY);
  }
  ...
  protected void extractLabels(@NotNull GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
    ...
    guiGraphics.text(this.font, this.title, GUI_CENTER + 8, MAIN_TOP - 12, textColor, false);
  }

  @Override
  public boolean mouseClicked(@NotNull MouseButtonEvent event, boolean doubleClick) {
    if (rootElement != null && rootElement.mouseClicked(event.x(), event.y(), event.button())) {
      return true;
    }
  ...
  public boolean mouseReleased(@NotNull MouseButtonEvent event) {
    if (rootElement != null && rootElement.mouseReleased(event.x(), event.y(), event.button())) {
      return true;
    }
  ...
  public boolean mouseDragged(@NotNull MouseButtonEvent event, double dragX, double dragY) {
    if (rootElement != null && rootElement.mouseDragged(
  ...
  public boolean keyPressed(@NotNull KeyEvent event) {
    if (rootElement != null && rootElement.keyPressed(event.key(), event.scancode(), event.modifiers())) {
      return true;
    }
  ...
  public boolean charTyped(@NotNull CharacterEvent event) {
    ...
    if (Character.isBmpCodePoint(codePoint) && rootElement != null &&
        rootElement.charTyped((char) codePoint, 0)) {
      return true;
    }
  ...
  private class PagesProvider implements MystGuiPageSurface.PagesProvider {
    ...
    private String getPageName(ItemStack page) {
      Identifier symbol = Page.getSymbol(page);
      if (symbol != null) {
    ...
    public void copy(MystGuiPageSurface.PositionableItem item) {
      Identifier symbol = Page.getSymbol(item.itemstack);
      if (symbol != null) {
        MystcraftNetwork.sendToServer(new ContainerActionPacket(
