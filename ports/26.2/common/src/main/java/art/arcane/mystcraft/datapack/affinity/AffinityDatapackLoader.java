...
public final class AffinityDatapackLoader {
   ...
   */
  public static void apply(@NotNull Map<Identifier, JsonElement> resources) {
    InkAffinity.clear();
    int loaded = 0;
    int skipped = 0;
  ...
  private static boolean parseAndRegister(Identifier id, JsonElement element) {
    ...
    Map<Identifier, Float> symbolWeights = readSymbolWeights(obj.getAsJsonObject("symbol_weights"));
    ...
    if (items != null) {
      for (JsonElement raw : items) {
        ...
        Identifier itemId = Identifier.tryParse(raw.getAsString());
    ...
    if (tags != null) {
      for (JsonElement raw : tags) {
        if (!raw.isJsonPrimitive()) continue;
        Identifier tagId = Identifier.tryParse(raw.getAsString());
  ...
  private static Map<Identifier, Float> readSymbolWeights(JsonObject obj) {
