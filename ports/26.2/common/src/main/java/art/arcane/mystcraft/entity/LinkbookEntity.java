...
/**
 * The Linkbook entity. Represents a dropped/placed linkbook in the world that
 * can be used for linking. Features damage/decay system for survival
 * mechanics.
 */
public class LinkbookEntity extends Entity {
  ...
  private static final float DROWNING_DAMAGE = 20.0f;
  ...
  protected void defineSynchedData(SynchedEntityData.Builder builder) {
    builder.define(BOOK_ITEM, ItemStack.EMPTY);
    builder.define(BOOK_HEALTH, MAX_HEALTH);
  }

  @Override
  protected void readAdditionalSaveData(ValueInput input) {
    setBookItem(input.read("Book", ItemStack.CODEC).orElse(ItemStack.EMPTY));
    setHealth(input.getFloatOr("Health", MAX_HEALTH));
  ...
  protected void addAdditionalSaveData(ValueOutput output) {
    ...
    if (!book.isEmpty()) {
      output.store("Book", ItemStack.CODEC, book);
    }
  ...
  /**
   * Activates the book for the given entity (called from
   * EntityBookActivatePacket). Performs the actual linking based on book type.
   */
  ...
  private void processGroundDecay() {
    ...
    if (!book.isEmpty() && tag != null && tag.getBooleanOr("NoDecay", false)) {
      return;
    }
