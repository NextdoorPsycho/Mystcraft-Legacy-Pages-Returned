...
public class AgeBuilder {
  ...
  private static final Block[] OVERWORLD_BLOCKS = {
      ...
      Blocks.DYED_TERRACOTTA.red(), Blocks.DYED_TERRACOTTA.orange(),
      Blocks.DYED_TERRACOTTA.yellow(), Blocks.DYED_TERRACOTTA.lightBlue(),
      Blocks.CONCRETE.white(), Blocks.CONCRETE.lightGray(),
      Blocks.CONCRETE.cyan(), Blocks.CONCRETE.blue(),
      ...
      Blocks.CLAY, Blocks.COPPER_BLOCK.weathering().unaffected(), Blocks.COPPER_BLOCK.weathering().oxidized(),
      Blocks.CUT_COPPER.weathering().unaffected(), Blocks.AMETHYST_BLOCK,
  ...
  private final List<Identifier> missingSymbols = new ArrayList<>();
  ...
  private void expandSymbols() {
    ...
    List<Identifier> terminals = new ArrayList<>();
    ...
    List<Identifier> expandedNames = tree.getExpanded(rand);
    ...
    for (Identifier name : expandedNames) {
  ...
  private void filterBiomesByTerrain(String terrainType) {
    ...
    if (isNether) {
      defaultBiome = Identifier.fromNamespaceAndPath("mystcraft", "biome_nether_wastes");
    } else if (isEnd) {
      defaultBiome = Identifier.fromNamespaceAndPath("mystcraft", "biome_the_end");
    } else {
      defaultBiome = Identifier.fromNamespaceAndPath("mystcraft", "biome_plains");
    }
  ...
  private void filterUnrequestedVoidTerrain() {
    ...
    if (removed && !hasTerrainSymbol()) {
      ...
      if (!candidates.isEmpty()) {
        ...
        fallback = SymbolRegistry.get(Identifier.fromNamespaceAndPath("mystcraft", "terrain_normal"));
      }
      if (fallback != null) {
        expandedSymbols.add(fallback);
  ...
  private void applyStarFissureDefault(AgeDirectorImpl director, Random rand) {
    ...
    if (!hasPopulator) {
      ...
      IPopulate populator = PopulatorRegistry.create(Identifier.fromNamespaceAndPath("mystcraft", "star_fissure"), params, director.getSeed());
      if (populator != null) {
  ...
  private void applyTerrainBlockBias(AgeDirectorImpl director, Random rand, float fallbackScale) {
    ...
    for (IAgeSymbol symbol : expandedSymbols) {
      Identifier name = symbol.getRegistryName();
  ...
  private void applyOreVariation(AgeDirectorImpl director, Random rand, float fallbackScale) {
      ...
      for (int i = 0; i < boostCount && pickedIndices.size() < EXTRA_ORE_SYMBOLS.length; i++) {
        ...
        if (pickedIndices.add(idx)) {
          IAgeSymbol oreSymbol = SymbolRegistry.get(
              Identifier.fromNamespaceAndPath("mystcraft", EXTRA_ORE_SYMBOLS[idx]));
          if (oreSymbol != null) {
    ...
    if (rand.nextFloat() < ORE_DISABLE_CHANCE * fallbackScale) {
      ...
      IAgeSymbol disableSymbol = SymbolRegistry.get(
          Identifier.fromNamespaceAndPath("mystcraft", DISABLE_ORE_SYMBOLS[idx]));
  ...
  private void applyTimescaleVariation(AgeDirectorImpl director, Random rand, float fallbackScale) {
    ...
    IAgeSymbol timescaleSymbol = SymbolRegistry.get(
        Identifier.fromNamespaceAndPath("mystcraft", TIMESCALE_SYMBOLS[selectedIdx]));
  ...
  private void applyEnvironmentEffects(AgeDirectorImpl director, Random rand, float fallbackScale) {
    ...
    IAgeSymbol envSymbol = SymbolRegistry.get(
        Identifier.fromNamespaceAndPath("mystcraft", ENV_EFFECT_SYMBOLS[selectedIdx]));
    if (envSymbol != null) {
  ...
  private void applyFeatureInjection(AgeDirectorImpl director, Random rand, float fallbackScale) {
    ...
    for (int i = 0; i < injectCount && !candidates.isEmpty(); i++) {
      ...
      IAgeSymbol featureSymbol = SymbolRegistry.get(
          Identifier.fromNamespaceAndPath("mystcraft", feature));
  ...
  private void applyBiomeChaos(AgeDirectorImpl director, Random rand, float fallbackScale) {
    ...
    for (int i = 0; i < injectCount; i++) {
      ...
      IAgeSymbol biomeSymbol = SymbolRegistry.get(
          Identifier.fromNamespaceAndPath("mystcraft", biomePath));
      if (biomeSymbol != null) {
