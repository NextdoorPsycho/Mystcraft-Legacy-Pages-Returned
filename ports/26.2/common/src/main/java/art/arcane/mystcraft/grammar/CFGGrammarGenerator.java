...
public final class CFGGrammarGenerator {
  ...
  private static final Map<Identifier, RankData> ranks = new ConcurrentHashMap<>();
  private static final Map<Identifier, List<CFGRule>> mappings = new ConcurrentHashMap<>();
  private static final Map<Identifier, List<CFGRule>> reverseLookup = new ConcurrentHashMap<>();
  private static volatile Map<Identifier, Map<Identifier, List<List<CFGRule>>>> shortestPaths = null;
   ...
   */
  public static void removeRulesForParent(Identifier parent) {
    ...
    for (CFGRule rule : rules) {
      for (Identifier value : rule.values()) {
   ...
   */
  public static List<CFGRule> getParentRules(Identifier token) {
   ...
   */
  public static List<CFGRule> getAllRules(Identifier token) {
   ...
   */
  public static CFGRule getRandomRule(Identifier token, Random rand) {
    List<CFGRule> rules = mappings.get(token);
    if (rules == null || rules.isEmpty()) {
      return null;
    }
   ...
   */
  public static List<Identifier> explore(Identifier token, Random rand) {
    ...
    for (Identifier produced : rule.values()) {
   ...
   * exists
   */
  public static List<List<CFGRule>> getShortestPaths(Identifier subtreeToken, Identifier nodeToken) {

    Map<Identifier, Map<Identifier, List<List<CFGRule>>>> localPaths = shortestPaths;
    if (localPaths == null) {
    ...
    Map<Identifier, List<List<CFGRule>>> allPaths = localPaths.get(subtreeToken);
    if (allPaths == null) {
      return null;
    }
   ...
   */
  public static RankData getRankData(Identifier parent) {
  ...
  private static void buildShortestPaths() {
    ...
    Map<Identifier, Map<Identifier, List<List<CFGRule>>>> tempPaths = new ConcurrentHashMap<>();

    int count = 0;
    int total = reverseLookup.size();
    for (Identifier token : reverseLookup.keySet()) {
  ...
  private static Map<Identifier, List<List<CFGRule>>> getOrCalculatePaths(
      Map<Identifier, Map<Identifier, List<List<CFGRule>>>> pathsMap,
      Identifier token) {

    Map<Identifier, List<List<CFGRule>>> allPaths = pathsMap.get(token);
    if (allPaths != null) {
      return allPaths;
    }
    ...
    Set<Identifier> exploredFromNode = new HashSet<>();
  ...
  private static void validateTokenPopulation() {
    ...
    int unpopulated = 0;
    for (Identifier token : terminalTokens) {
      List<CFGRule> rules = mappings.get(token);
      if (rules == null) {
   ...
   */
  public static void testShortestPaths(Identifier token1, Identifier token2) {
  ...
  private record VisitPair(Identifier target, List<CFGRule> path) {
  }
