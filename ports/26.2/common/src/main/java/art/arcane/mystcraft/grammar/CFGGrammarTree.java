...
public class CFGGrammarTree {
  ...
  private List<Identifier> terminals;

  /**
   * Creates a new grammar tree with the given root token.
   */
  public CFGGrammarTree(Identifier root) {
   ...
   */
  public void parseTerminals(List<Identifier> terminals, Random rand) {
    this.terminals = Collections.unmodifiableList(new ArrayList<>(terminals));

    for (int i = terminals.size(); i > 0; i--) {
      Identifier terminal = terminals.get(i - 1);
   ...
   */
  public List<Identifier> getExpanded(Random rand) {
    ...
    Map<Integer, List<Identifier>> insertLeft = new HashMap<>();
    Map<Integer, List<Identifier>> insertRight = new HashMap<>();
    getInsertions(root, insertLeft, insertRight);

    for (int i = out.size() + 1; i > 0; i--) {
      List<Identifier> products = insertRight.get(i - 1);
      if (products != null) {
  ...
  private void getInsertions(GrammarNode node,
                             Map<Integer, List<Identifier>> insertLeft,
                             Map<Integer, List<Identifier>> insertRight) {
  ...
  private boolean checkForLoop(Identifier parent, GrammarNode subroot) {
  ...
  private void expandUnexploredNode(int index, CFGRule rule) {
    ...
    List<Identifier> products = rule.values();
    for (int i = products.size(); i > 0; i--) {
      Identifier product = products.get(i - 1);
  ...
  private static class GrammarNode {
    ...
    GrammarNode(Identifier token, int pos) {
      this.token = token;
      this.leftPos = pos;
      this.rightPos = pos;
