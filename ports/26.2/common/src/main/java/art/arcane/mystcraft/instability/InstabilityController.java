...
public class InstabilityController implements InstabilityDirector {
  ...
  public void tick(List<LevelChunk> chunks) {
    ...
    float multiplier = Math.max(0.0f, MystcraftConfig.instabilityMultiplier.get().floatValue());
    ...
    for (IEnvironmentalEffect effect : effects) {
      ...
      if (level.getRandom().nextFloat() < fractionalRun) {
        tickEffect(effect, chunks, instability);
      }
    }
  }
  ...
  private void tickEffect(IEnvironmentalEffect effect, List<LevelChunk> chunks, float instability) {
    LevelChunk chunk = chunks.get(level.getRandom().nextInt(chunks.size()));
    try {
