...
public class BoosterPackItem extends Item {
  ...
  public static InkBlend getAffinity(@NotNull ItemStack stack) {
    ...
    if (tag == null || !NbtCompat.contains(tag, TAG_AFFINITY, Tag.TAG_COMPOUND))
      return null;
    InkBlend blend = InkBlend.fromTag(tag.getCompoundOrEmpty(TAG_AFFINITY));
  ...
  public InteractionResult use(@NotNull Level level, @NotNull Player player, @NotNull InteractionHand hand) {
    ...
    if (!level.isClientSide()) {
      ...
      if (!(player instanceof net.minecraft.server.level.ServerPlayer serverPlayer) || serverPlayer.connection != null) {
        player.sendOverlayMessage(Component.translatable("item.mystcraft.booster.opened", pages.size()));
      }
    }
