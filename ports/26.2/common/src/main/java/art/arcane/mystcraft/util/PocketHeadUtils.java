...
public final class PocketHeadUtils {
  ...
  private static GameProfile resolveProfile(MinecraftServer server, UUID owner) {
    try {
      return server.services().profileResolver().fetchById(owner)
          .orElseGet(() -> new GameProfile(owner, ""));
    } catch (RuntimeException e) {
      Mystcraft.LOGGER.debug("[PocketHead] Profile lookup failed for {}", owner, e);
      return new GameProfile(owner, "");
    }
  }
  ...
  private static GameProfile resolveProfileByName(MinecraftServer server, String name) {
    try {
      return server.services().profileResolver().fetchByName(name).orElse(null);
    } catch (RuntimeException e) {
      Mystcraft.LOGGER.debug("[PocketHead] Profile lookup failed for {}", name, e);
      return null;
    }
  }
  ...
  private static String getSkinUrl(MinecraftServer server, GameProfile profile) {
    ...
    if (sessionService == null) {
      Mystcraft.LOGGER.debug("[PocketHead] Session service is unavailable for {}", profile.name());
      return null;
    }
    try {
      ...
      Mystcraft.LOGGER.warn("[PocketHead] Failed to resolve textures for {}", profile.name(), e);
      return null;
    }
  }
