...
public class MapGenFloatingIslands implements ITerrainAlteration {
          ...
          for (int y = Math.max(minBuild, stemBottom); y <= Math.min(maxBuild, stemTop); y++) {
            ...
            if (horizDist <= Math.max(1.0, taperRadius)) {
              ...
              if (chunk.getBlockState(pos).isAir()) {
                chunk.setBlockState(pos, Blocks.MUSHROOM_STEM.defaultBlockState(), 0);
              }
            }
          }
        }
            ...
            if (dripNoise > 0.3) {
              ...
              for (int dy = 1; dy <= dripLength; dy++) {
                ...
                if (chunk.getBlockState(pos).isAir()) {
                  ...
                  chunk.setBlockState(pos, dripBlock, 0);
                }
              }
            }
          }
    ...
    for (int x = localMinX; x <= localMaxX; x++) {
      ...
      for (int z = localMinZ; z <= localMaxZ; z++) {
        ...
        for (int y = yMin; y <= yMax; y++) {
          pos.set(x, y, z);
          if (!chunk.getBlockState(pos).isAir()) continue;
          chunk.setBlockState(pos, pickShardBlock(noise, worldX, y, worldZ), 0);
        }
      }
    }
  }
  ...
  private BlockState pickMesaBlock(int y, int centerY, long islandSeed, IslandNoise noise, int worldX, int worldZ) {
    ...
    return switch (band) {
      ...
      case 1 -> Blocks.DYED_TERRACOTTA.orange().defaultBlockState();
      case 2 -> Blocks.DYED_TERRACOTTA.white().defaultBlockState();
      case 3 -> Blocks.DYED_TERRACOTTA.yellow().defaultBlockState();
      case 4 -> Blocks.DYED_TERRACOTTA.red().defaultBlockState();
      case 5 -> Blocks.DYED_TERRACOTTA.brown().defaultBlockState();
      default -> Blocks.DYED_TERRACOTTA.lightGray().defaultBlockState();
    };
  }
  ...
  private void decorateSurface(ChunkAccess chunk, BlockPos.MutableBlockPos pos,
    ...
    if (belowBlock.is(Blocks.MOSS_BLOCK) && decorNoise > 0.4) {
      chunk.setBlockState(above, Blocks.MOSS_CARPET.defaultBlockState(), 0);
      return;
    }
    ...
    if (decorNoise > 0.6) {
      ...
      chunk.setBlockState(above, flower, 0);
    } else if (decorNoise > 0.35) {

      if (patchType > 0.0) {
        chunk.setBlockState(above, Services.PLATFORM.getShortGrassBlock().defaultBlockState(), 0);
      } else {
        chunk.setBlockState(above, Blocks.FERN.defaultBlockState(), 0);
      }
    }
  }
  ...
  private void decorateMushroomSurface(ChunkAccess chunk, BlockPos.MutableBlockPos pos,
    ...
    if (decorNoise > 0.4) {
      chunk.setBlockState(above, Blocks.RED_MUSHROOM.defaultBlockState(), 0);
    } else if (decorNoise > 0.2) {
      chunk.setBlockState(above, Blocks.BROWN_MUSHROOM.defaultBlockState(), 0);
    }
  }
  ...
  private void maybeDecorateUnderside(ChunkAccess chunk, BlockPos.MutableBlockPos pos,
    ...
    double decorNoise = noise.detail.sample2D(worldX * 0.15, worldZ * 0.15, 2.0, 0.5);
    if (decorNoise > 0.5) {
      chunk.setBlockState(below, Blocks.HANGING_ROOTS.defaultBlockState(), 0);
    } else if (decorNoise < -0.45) {
      chunk.setBlockState(below, Blocks.POINTED_DRIPSTONE.defaultBlockState(), 0);
    }
  }
  ...
  private void maybeGlowUnderside(ChunkAccess chunk, BlockPos.MutableBlockPos pos,
    ...
    if (glowNoise > 0.6) {
      ...
      chunk.setBlockState(pos, Blocks.SHROOMLIGHT.defaultBlockState(), 0);
    } else if (glowNoise > 0.45) {
      chunk.setBlockState(pos, Blocks.GLOWSTONE.defaultBlockState(), 0);
    }
  }
    ...
    for (int t = 0; t < treeCount; t++) {
      ...
      for (int ty = 1; ty <= trunkHeight; ty++) {
        ...
        if (chunk.getBlockState(treePos).isAir()) {
          chunk.setBlockState(treePos, logBlock, 0);
        }
      }
        ...
        for (int lz = -leafRadius; lz <= leafRadius; lz++) {
          for (int ly = -1; ly <= leafRadius; ly++) {
            ...
            if (leafLocalX < 0 || leafLocalX > 15 || leafLocalZ < 0 || leafLocalZ > 15)
              continue;
            if (leafY >= chunk.getMaxY() || leafY <= chunk.getMinY())
              continue;

            treePos.set(leafLocalX, leafY, leafLocalZ);
            if (chunk.getBlockState(treePos).isAir()) {
              chunk.setBlockState(treePos, leafBlock, 0);
            }
          }
        }
      }
    ...
    for (int px = -pondRadius; px <= pondRadius; px++) {
      for (int pz = -pondRadius; pz <= pondRadius; pz++) {
        ...
        if (isRim) {
          ...
          if (foundSurface - 1 > chunk.getMinY() && !chunk.getBlockState(pos).isAir()) {
