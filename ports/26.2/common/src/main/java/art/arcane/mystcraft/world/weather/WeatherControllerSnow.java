...
/**
 * Weather controller that provides constant snow-producing precipitation. Rain
 * is always active but biomes are treated as cold (for snow). This controller
 * sets the weather, but temperature handling is done at biome level.
 */
public class WeatherControllerSnow implements IWeatherController {
  ...
  public void updateWeather(ServerLevel level) {
    ...
    if (!level.isRaining()) {
      level.getServer().setWeatherParameters(0, 6000, true, false);
    }
  }
  ...
  /**
   * Snow controller should force cold temperatures. This is checked by
   * biome-related code to determine if precipitation falls as snow.
   *
